...
First, import a new module at the top:
...
@app.route("/static/react-client/dist/<path:filename>")
...
# Route for redirecting all non-logged in users to the login form:
@app.route("/")
@decorators.jwt_or_login
def home():
    if os.getenv("ENVIRONMENT") != "development":
        return send_from_directory(
        app.root_path + "/static/react-client", "index.html"
    )
    else:
        return f'''
...
# Modified API tester that embeds the user's JWT:
@app.route("/api")
@app.route("/api/")
@decorators.jwt_or_login
def api_docs():
...
This should create a new directory called "dist" that will house your compiled React app. When you're done, restart your flask app and navigate to <a href="http://127.0.0.1:5000" target="_blank">http://127.0.0.1:5000</a>. You should see your react app running.
